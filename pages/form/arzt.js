...
const BehandlungsFormular = (props) => {
  ...
  const { data: arztData, error: arztError } = useSWR(
    `${baseURL}/s4/arzte/${session.id}`,
    fetcher
  );

  const [date, setDate] = useState(new Date());
  const [arztbrief, setArztbrief] = useState(null);
  const [blutwerte, setBlutwerte] = useState(null);
  const [roentgen, setRoentgen] = useState(null);
  const [befunde, setBefunde] = useState(null);
  const [rechnung, setRechnung] = useState(null);
  const [sonstiges, setSonstiges] = useState(null);
