...
export const getAllUser = () => {
  ...
  const { data: assData, error: assError } = useSWR(
    `${baseURL}/s4/assistenten/`,
    fetcher
  );

  const { data: verwData, error: verwError } = useSWR(
    `${baseURL}/s1/verwaltungsmitarbeiter/`,
    fetcher
  );

  if (patientData && arztData && assData && verwData) {
    patientData.map((data) => (data.role = "Patient"));
    arztData.map((data) => (data.role = "Arzt"));
    assData.map((data) => (data.role = "Assistent"));
    verwData.map((data) => (data.role = "Verwaltung"));
